...
<div class="container">
  ...
  <c:when test="${not empty page.getContent()}">
    ...
    <c:url var="firstUrl" value="/list/1${param}?from=${from}&to=${to}&comp=${comp}"/>
    <c:url var="lastUrl" value="/list/${page.totalPages}?from=${from}&to=${to}&comp=${comp}"/>
    <c:url var="prevUrl" value="/list/${currentIndex - 1}?from=${from}&to=${to}&comp=${comp}"/>
    <c:url var="nextUrl" value="/list/${currentIndex + 1}?from=${from}&to=${to}&comp=${comp}"/>
    ...
    <ul class="pagination">
      ...
      <c:forEach var="i" begin="${beginIndex}" end="${endIndex}">
        ...
        <c:choose>
          ...
          <c:otherwise>
            <li><a href="${pageUrl}?from=${from}&to=${to}&comp=${comp}"><c:out value="${i}"/></a></li>
          </c:otherwise>
        </c:choose>
      </c:forEach>
      <c:choose>
